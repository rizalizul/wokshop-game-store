<?php
  ...
  $query = "SELECT * FROM game WHERE id_game='$getIdGame';";
      ...
      <div class="row justify-content-center">
        ...
        ?>
          <form action="" method="POST"  enctype="multipart/form-data">
            <h2 class="text-center border-head">Edit Game</h2>
              <hr class="dropdown-divider mb-3" />            
            <div class="mb-3">
              <label class="form-label">Nama Game</label>
              <input type="text" name="nama_game" class="form-control" placeholder="Nama Game" value="<?php echo $result['nama_game'] ?>" required />
            </div>
            <div class="mb-3">
              <label class="form-label">Nama Item Game</label>
              <input type="text" name="nama_item_game" class="form-control" placeholder="Nama Item Game" value="<?php echo $result['nama_item_game'] ?>" required />
            </div>
...
<?php
    ...
    if ($file_size < 64000 and ($file_type =='image/jpeg' or $file_type == 'image/png'))
    {
        ...
        $query1 = mysqli_query($con, "UPDATE game 
        SET nama_game='$nama_game', nama_item_game='$nama_item_game',nama_image_game='$file_name', gambar='$image'
        WHERE id_game='$getIdgame'");
